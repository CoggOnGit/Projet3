...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (editButton) {
    // Ouvre l'overlay lors du clic sur le bouton d'édition
  ...
  if (closeButton) {
    // Ferme l'overlay lors du clic sur le bouton de fermeture
...
/* AFFICHER DYNAMIQUEMENT LES CATEGORIES DANS LE SELECT */
fetch("http://localhost:5678/api/categories")
.then(response => response.json())
.then(data => {
  const uploadCategorieSelect = document.getElementById("uploadCategorie");
  ...
  data.forEach(category => {
    ...
    uploadCategorieSelect.appendChild(option);
  });
})
.catch(error => {
  console.error(error);
